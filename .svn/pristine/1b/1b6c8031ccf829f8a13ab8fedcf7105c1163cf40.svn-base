...
public class ControladorJuego {
	...
	public void comenzarPartidaCargada() throws Exception {
		...
		if (!config.getHumano2()) {
			...
			try {
                ...
                contPartida.setGanador( ControladorAlgo.partidaGanada(tablero));
	}
	...
	private void rellenarTablero() throws Exception {
		...
		if (fichasJ1.get(0).getx() == fichasJ2.get(0).getx() && fichasJ1.get(0).gety() == fichasJ2.get(0).gety()) {
	...
	public void deshacer(int n) throws Exception {
		...
		if (config.getDeshacer()) {
			if (!pila.empty() && n > 0) {
				int i = 0;
				Posicion o = new Posicion(0, 0);
				while (i < n) {
					if (jugadorTurnoActual() == 1) {
						o = jugador2.getfichas().get(jugador2.getfichas().size() - 1);
						jugador2.quitarUltimaFicha();
					} else if (jugadorTurnoActual() == 2) {
						o = jugador1.getfichas().get(jugador1.getfichas().size() - 1);
						jugador1.quitarUltimaFicha();
					}
					if (contPartida.getturnos() == 2 && contPartida.getCambio()) {
						contPartida.setCambio(false);
						contPartida.pilaPush(o);
					}
